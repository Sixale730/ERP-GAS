...
- `cotizacion_a_factura` **siempre descuenta** (sin idempotency). Migración `20260513_001_cotizacion_a_factura_sin_idempotency.sql` quitó el check porque era demasiado restrictivo: bloqueaba el descuento en OVs legacy cuyo inventario ya había sido compensado por un ajuste manual posterior (caso real OV-00071 13/05/2026 que dejó GP-RD-C2 en 17 cuando debía ser 9). Las OVs legacy aún no facturadas que ya descontaron al pasar a OV requieren un ajuste compensatorio manual ANTES de facturar (sumar al inventario lo que la OV gastó), o aceptar que el trigger `trg_inventario_no_negativa` bloqueará la facturación si dejaría el saldo profundizando un negativo.
- Protección a nivel BD: trigger `trg_inventario_no_negativa` (BEFORE INSERT/UPDATE de `cantidad` en `erp.inventario`, función `erp.fn_inventario_no_negativa`). Reglas: en INSERT exige `cantidad >= 0`; en UPDATE rechaza solo si `NEW.cantidad < 0 AND NEW.cantidad < OLD.cantidad` (es decir, salidas que profundizan o crean un saldo negativo). Permite **entradas** a inventario negativo (recepciones, restauraciones, ajustes positivos) aunque el saldo final siga siendo negativo, porque van en la dirección correcta. Migración `20260512_002_inventario_no_negativa_trigger.sql` reemplazó el CHECK estático original (`20260511_001`), que bloqueaba erróneamente recepciones de SKUs con saldo legacy negativo (caso real: GP-RD-C2 en −23 no podía recibir 10 porque −13 seguía siendo `< 0`).
