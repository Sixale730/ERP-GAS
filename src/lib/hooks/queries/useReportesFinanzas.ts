...
export function useABCClientes(
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      // Lanzar ambas queries en paralelo si aplica
...
export function useABCProductos(
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      // Lanzar queries de facturas y POS en paralelo
      const queries: Promise<void>[] = []

      const acumular = (items: { producto_id: string; subtotal: number; cantidad: number }[]) => {
      ...
      if (fuente === 'facturas' || fuente === 'both') {
        queries.push((async () => {
          let fq = supabase.schema('erp').from('facturas').select('id')
            .eq('organizacion_id', orgId!).not('status', 'eq', 'cancelada').limit(5000)
      ...
      if (fuente === 'pos' || fuente === 'both') {
        queries.push((async () => {
          let vq = supabase.schema('erp').from('ventas_pos').select('id')
            .eq('status', 'completada').eq('organizacion_id', orgId!).limit(5000)
...
export function useFlujoEfectivo(
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const supabase = getSupabaseClient()
      // Queries de pagos e ingresos en paralelo
      let pq = supabase.schema('erp').from('pagos').select('fecha, monto').eq('organizacion_id', orgId!).limit(10000)
      ...
      let oq = supabase.schema('erp').from('ordenes_compra').select('fecha, total').eq('organizacion_id', orgId!)
        .in('status', ['recibida', 'parcialmente_recibida']).limit(10000)
      if (fechaDesde) oq = oq.gte('fecha', fechaDesde)
      if (fechaHasta) oq = oq.lte('fecha', fechaHasta)

      const [{ data: pagos }, { data: ordenes }] = await Promise.all([pq, oq])
...
export function useEstadoResultados(
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const supabase = getSupabaseClient()
      let fq = supabase.schema('erp').from('facturas').select('id, subtotal').eq('organizacion_id', orgId!).not('status', 'eq', 'cancelada').limit(5000)
      if (fechaDesde) fq = fq.gte('fecha', fechaDesde)
      if (fechaHasta) fq = fq.lte('fecha', fechaHasta)
      const { data: facturas } = await fq
      ...
      if (facturaIds.length > 0) {
        // Join items con productos para obtener costo_promedio en una sola query (elimina N+1)
        const { data: items } = await supabase.schema('erp').from('factura_items')
          .select('producto_id, cantidad, productos (costo_promedio)')
          .in('factura_id', facturaIds)
        for (const it of items || []) {
          if (!it.producto_id) continue
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          const costo = Number((it as any).productos?.costo_promedio || 0)
          costoVentas += Number(it.cantidad || 0) * costo
          numProductos += Number(it.cantidad || 0)
        }
      }
